	...
	MyUeList &my_UE_list)
{
	...
	for (int ue_index = 0; ue_index < my_UE_list.getSize(); ue_index++)
	{
		...
		if (beta_u[ue_index] < RF_AP_Num)
		{
			...
			//by eq.14
			double bw = (1 / my_UE_listgetElement(ue_index).Get_Avg_Throughput()) / Reciprocal_Sum_Of_RF_UE;
...
void Benchmark_Update_APSelection_Result(MyUeList &my_UE_list, std::vector<int> &beta_u, std::vector<int> &served_UE_Num, std::vector<std::vector<int>> &AP_Association_Matrix)
{
	...
	//重算
	for (int ue_index = 0; ue_index < my_UE_list.size(); ue_index++)
...
//2. 歷史平均avg data rate
//3. 此輪滿意度
void Benchmark_Update_RA_Result(MyUeList &my_UE_list, std::vector<double> &Throughput)
{
